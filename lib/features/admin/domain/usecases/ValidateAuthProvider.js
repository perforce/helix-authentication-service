...
// Provider properties whose values are server filesystem paths. These are only
// meaningful for classic, operator-managed configuration (e.g. set via the
// OIDC_*/SAML_* environment settings). They must never be accepted from the
// admin HTTP API, where an authenticated client could otherwise point them at
// arbitrary files and have the contents read back (see HAS-677); API clients
// must supply the corresponding inline value instead.
export const providerFilePathFields = Object.freeze([
  'clientCertFile',
  'clientKeyFile',
  'clientSecretFile',
  'idpCertFile',
 ...
 *
 * @param {Object} provider - object to be validated as a provider.
 * @param {Object} [options] - validation options.
 * @param {boolean} [options.rejectFilePaths] - if true, reject any provider
 *   that carries a filesystem-path property; use for untrusted API input.
 * @return {String} error message or null.
 * @throws {Error} if validation fails for any reason.
 */
export default () => {
  return (provider, { rejectFilePaths = false } = {}) => {
    ...
    // For untrusted API input, refuse filesystem-path properties outright so
    // the caller cannot read arbitrary server files; the inline value property
    // (e.g. clientSecret) must be used instead.
    if (rejectFilePaths) {
      for (const field of providerFilePathFields) {
        if (field in provider) {
          return `file path property not permitted: ${field}`
        }
      }
    }
