...
class HelixCredentialsRepository extends CredentialsRepository {
  constructor({ settingsRepository }) {
    ...
    this.p4tickets = settingsRepository.get('P4TICKETS')
    this.p4trust = settingsRepository.get('P4TRUST')
  }
  ...
  async verify(username, password) {
    ...
    const p4 = new P4({
      P4PORT: this.p4port,
      P4USER: username,
      P4TICKETS: this.p4tickets,
      P4TRUST: this.p4trust
    })
    ...
    if (findData(loginOut, 'Password invalid')) {
      // On a secure-by-default server (security=3) the server reports an
      // invalid password rather than revealing whether the user exists; either
      // way the credentials did not match.
      return false
    }
    if (loginOut.error) {
