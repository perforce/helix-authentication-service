...
import { corsConfiguration, getServiceURI } from 'helix-auth-svc/lib/server.js'
import { providerFilePathFields } from 'helix-auth-svc/lib/features/admin/domain/usecases/ValidateAuthProvider.js'
...
router.use(cors(corsConfiguration(settings)))
...
router.post('/', passport.authenticate('jwt', { session: false }), async (req, res) => {
  if (req.is('application/json')) {
    ...
    // reject filesystem-path provider properties supplied via the API, which
    // could otherwise be used to read arbitrary server files (see HAS-677)
    const badField = findProviderFilePath(req.body.AUTH_PROVIDERS)
    if (badField) {
      return res.status(400).json({
        status: 400,
        message: `file path property not permitted: ${badField}`
      })
    }
    try {
...
// Return the name of the first filesystem-path property found among the given
// providers, or undefined if there are none. Used to reject such properties
// when they arrive from the admin API, where they would allow reading arbitrary
// server files.
function findProviderFilePath(providers) {
