...
router.post('/apply', passport.authenticate('jwt', { session: false }), async (req, res) => {
  ...
  if (applyChanges) {
    applyChanges()
    // simulate restarting the process by clearing the in-memory repository,
    // but only when _not_ running the unit tests
...
async function serializeProviders(providers) {
  const temporaryRepository = container.resolve('temporaryRepository')
  const convertToProviders = container.resolve('convertToProviders')
  ...
  await convertToProviders(temporaryRepository, { shadow: true })
