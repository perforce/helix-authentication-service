    ...
    if (spEntityId in idpConfig) {
      ...
      if ('acsUrl' in spConfig) {
        ...
        if (!expectedAcsUrl.startsWith(acsUrl)) {
          ...
          errors.push(`saml.sp.assertionConsumerService.url should be ${cleanUrl}`)
        }
        ...
        if (acsUrl.match(expectedRegex) === null) {
          ...
          errors.push(`saml.sp.assertionConsumerService.url should match ${cleanUrl}`)
        }
        ...
        if (!found) {
          ...
          errors.push(`saml.sp.assertionConsumerService.url should be one of ${cleanUrls}`)
        }
      } else {
