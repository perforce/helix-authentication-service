...
module.exports = ({ requestRepository, userRepository }) => {
  ...
  return (requestId, timeout, interval) => {
    assert.ok(requestId, 'request identifier must be defined')
    const request = requestRepository.get(requestId)
    if (request) {
      const requestTimeout = timeout || 10000
      const intervalTimeout = interval || 1000
      return new Promise((resolve, reject) => {
        let user = userRepository.take(request.userId)
        if (user) {
          resolve(user)
        } else {
          // wait for the data to become available
          const timer = setInterval(() => {
            user = userRepository.take(request.userId)
            if (user) {
              clearInterval(timer)
              resolve(user)
            }
