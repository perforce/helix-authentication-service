...
async function loadSamlOptions (req, forceAuthn) {
  ...
  const certificate = await readFile(container.resolve('serviceKey'), 'utf-8')
...
router.get('/metadata', async (req, res) => {
  ...
  try {
    const publicCert = await readFile(container.resolve('serviceCert'), 'utf-8')
    const privateKey = await readFile(container.resolve('serviceKey'), 'utf-8')
    const serviceURI = server.getServiceURI(settings)
    const spEntityId = settings.get('SAML_SP_ENTITY_ID')
    const spIssuer = settings.get('SAML_SP_ISSUER')
    const audience = settings.get('SAML_SP_AUDIENCE')
    const keyAlgo = settings.get('SP_KEY_ALGO')
    // Changing these defaults, especially the issuer, is risky as there are
    // existing installations that might suddenly break when the service is
    // upgraded. Additionally, these values are documented in the admin guide.
    const options = {
      callbackUrl: serviceURI + '/saml/sso',
      logoutCallbackUrl: serviceURI + '/saml/slo',
      issuer: spEntityId || spIssuer || 'https://has.example.com',
      audience,
      decryptionPvk: privateKey,
      privateKey,
      signatureAlgorithm: keyAlgo || 'sha256',
...
router.get('/login/:id', (req, res, next) => {
  ...
  next()
}, ensureStrategy, passport.authenticate('saml', { failureRedirect: '/saml/login_failed' }))
  ...
  if (request) {
    ...
    if (request.userId === clientId) {
      ...
      try {
        ...
        logger.error('saml: error building response:', err)
        res.render('error', {
