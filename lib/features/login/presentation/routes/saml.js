...
function loadSamlOptions () {
  ...
  const samlOptions = {
    ...
    authnContext,
    wantAssertionsSigned: getTruthySetting('SAML_WANT_ASSERTION_SIGNED', false),
    wantAuthnResponseSigned: getTruthySetting('SAML_WANT_RESPONSE_SIGNED', false),
    disableRequestedAuthnContext: getTruthySetting('SAML_DISABLE_CONTEXT', false),
      ...
      findRequest(req.session.requestId || 'none').then((request) => {
        ...
        fetchSamlMetadata(options).then((config) => {
          ...
          if (Array.isArray(config.cert)) {
            ...
            return done(new Error('IdP certificate is required, please set IDP_CERT_FILE'))
          }
...
function getTruthySetting (name, fallback) {
