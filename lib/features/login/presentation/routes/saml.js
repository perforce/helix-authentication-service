...
function getPostURL (req, callback) {
  ...
  import(idpConfFile).then((module) => {
    const idpConfig = module.default
    const issuer = req.session.authnRequest.issuer
    if (issuer in idpConfig) {
      if ('acsUrl' in idpConfig[issuer]) {
      ...
      } else if ('acsUrlRe' in idpConfig[issuer]) {
        const urlre = idpConfig[issuer].acsUrlRe
        logger.debug('saml: POST URL regex %s for %s', urlre, issuer)
        if (urlre) {
          callback(null, urlre, true)
          return
        }
      }
    }
    ...
    logger.error('saml: failed to load IdP configuration', err)
    callback(err)
  })
}
...
router.get('/success', checkAuthentication, async (req, res, next) => {
  ...
  if (request) {
    ...
    if (request.userId === 'SAML:IdP:marker') {
      ...
      getPostURL(req, (err, url, isRegex) => {
        if (err) {
          res.render('error', {
          ...
          const urlMatches = isRegex ?
            moreOptions.recipient.match(url) !== null :
            moreOptions.recipient === url
          if (urlMatches) {
          ...
          res.render('error', {
            message: 'SAML 1-step neither failed nor suceeded',
            error: new Error('both err and url were undefined')
          })
        }
      })
    } else {
  ...
  return new Promise((resolve, reject) => {
    if (process.env.SAML_IDP_METADATA_FILE) {
      fs.readFile(process.env.SAML_IDP_METADATA_FILE, 'utf8', (err, data) => {
        if (err) {
          reject(new Error(`saml: error reading IdP metadata file: ${err}`))
        } else {
