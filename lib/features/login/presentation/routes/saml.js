...
router.get('/metadata', (req, res) => {
  const signingCert = fs.readFileSync(container.resolve('serviceCert'), 'utf-8')
...
router.get('/login', (req, res, next) => {
  ...
  samlp.parseRequest(req, (err, data) => {
    if (err) {
      ...
      // specify our legacy SAML request identifier
      startRequest('SAML:legacy:placeholder', forceAuthn, data.id)
...
router.get('/success', checkAuthentication, (req, res, next) => {
  ...
  if (request) {
    ...
    if (request.userId === 'SAML:legacy:placeholder') {
...
// need it to be validated, as well as fold in the available user profile data.
