...
router.get('/login/:id', (req, res, next) => {
  ...
  // logger.debug('saml: login remote address: %s', req.socket.remoteAddress)
  next()
    ...
    if (process.env.SAML_IDP_METADATA_FILE) {
      fs.readFile(process.env.SAML_IDP_METADATA_FILE, 'utf8', (err, data) => {
        if (err) {
          ...
          resolve(readIdpMetadata(reader))
        }
      })
...
function readIdpMetadata (reader) {
  const samlOptions = loadSamlOptions()
  // Some services (azure) will advertise multiple certs but the last one is not
  // necessarily valid, so pass everything every time.
