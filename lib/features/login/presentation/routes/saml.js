...
if (process.env.SAML_IDP_METADATA_FILE === 'undefined' && process.env.SAML_IDP_METADATA_URL === 'undefined' && process.env.SAML_IDP_SSO_URL === 'undefined') {
...
function checkStrategy (req, res, next) {
  ...
  if (process.env.SAML_IDP_METADATA_FILE || process.env.SAML_IDP_METADATA_URL || process.env.SAML_IDP_SSO_URL) {
  ...
  return new Promise((resolve, reject) => {
    if (process.env.SAML_IDP_METADATA_FILE) {
      fs.readFile(process.env.SAML_IDP_METADATA_FILE, 'utf8', (err, data) => {
        if (err) {
          logger.error('saml: error reading IdP metadata: %s', err)
          resolve(samlOptions)
        } else {
