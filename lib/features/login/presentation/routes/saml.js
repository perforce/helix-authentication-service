...
const strategy = new MultiSamlStrategy(
  {
    getSamlOptions: (req, done) => {
      ...
      const force = request && request.forceAuthn
...
router.get('/login', (req, res, next) => {
  ...
  samlp.parseRequest(req, (err, data) => {
    if (err) {
      ...
      startRequest(data.id, { id: 'SAML:legacy:placeholder' })
...
router.get('/success', checkAuthentication, (req, res, next) => {
  ...
  if (request) {
    ...
    if (request.id === 'SAML:legacy:placeholder') {
      ...
      logger.debug('saml: mapping %s to result %o', request.id, req.user)
