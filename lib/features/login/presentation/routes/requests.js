...
router.get('/new/:userId', async (req, res, next) => {
  ...
  const forceSetting = req.query.forceAuthn || process.env.FORCE_AUTHN || 'false'
  const forceAuthn = forceSetting === 'true' ? true : false
  ...
  const loginUrl = await generateLoginUrl(serviceURI, request.id, instanceId)
