...
const common = include('lib/features/login/presentation/routes/common')
...
router.get('/status/:requestId', async (req, res, next) => {
  ...
  if (!isSecure || authorized) {
    ...
    // The cert.subject and cert.issuer have null prototypes and a CN property
    // so printing the object yields no more information than just the CN.
    logger.warning('requests: certificates for %s from %s are not permitted',
