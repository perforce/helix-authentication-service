...
router.get('/new/:userId', async (req, res, next) => {
  ...
  const forceAuthn = await getForceAuthn(req.query)
...
async function getForceAuthn(query) {
  if (query.providerId) {
    const getAuthProviders = container.resolve('getAuthProviders')
    const providers = await getAuthProviders()
    const provider = providers.find((e) => e.id === query.providerId)
    if (provider && provider.forceAuthn) {
      return true
    }
  } else {
    return assessTruth(query.forceAuthn || settings.get('FORCE_AUTHN'))
  }
  return false
}
