...
export default ({ getDomainLeader, getDomainMembers, entityRepository }) => {
  ...
  return async (groupId, patch, domain) => {
    ...
    const group = await entityRepository.getGroup(groupId, leader)
    if (group) {
      ...
      if (!group.equals(patched)) {
        ...
        const updated = await entityRepository.updateGroup(patched, leader)
        const members = getDomainMembers(domain)
        for (const member of members) {
          await entityRepository.updateGroup(patched, member)
        }
        return updated
      }
      return group
