...
export default ({ getDomainLeader, getDomainMembers, entityRepository, settingsRepository }) => {
  ...
  return async (userId, patch, domain) => {
    ...
    const user = await entityRepository.getUser(userId, leader)
    if (user) {
      ...
      if (!user.equals(patched)) {
        ...
        const updated = entityRepository.updateUser(patched, leader)
        for (const member of members) {
          await entityRepository.updateUser(patched, member)
        }
        return updated
      }
      return user
