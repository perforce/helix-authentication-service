...
export default ({ getDomainLeader, getDomainMembers, entityRepository }) => {
  ...
  return async (group, domain) => {
    ...
    const existing = await entityRepository.getGroup(group.displayName, leader)
    if (existing) {
      throw new Error('group already exists')
    }
    const addedGroup = await entityRepository.addGroup(group, leader)
    const members = getDomainMembers(domain)
    for (const member of members) {
      if ( member.p4port != leader.p4port) {
        await entityRepository.addGroup(group, member)
      }
    }
    return addedGroup
  }
