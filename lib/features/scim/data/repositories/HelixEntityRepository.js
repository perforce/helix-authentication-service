...
class HelixEntityRepository extends EntityRepository {
  ...
  async makeP4 () {
    if (!this.config.password) {
      throw new Error('P4PASSWD not specified')
    }
    const p4 = new P4({
      P4PORT: this.config.port,
      P4USER: this.config.user
    })
    const loginOut = await p4.cmd('login', this.config.password)
    if (loginOut.error) {
  ...
  async getUsers (query) {
    ...
    return usersOut.stat.map((e) => {
      ...
      if (keyMap[key]) {
        if (keyMap[key].userName) {
          // restore the original userName
          user.userName = keyMap[key].userName
        }
        if (keyMap[key].externalId) {
          user.externalId = keyMap[key].externalId
        }
        if ('active' in keyMap[key]) {
          user.active = keyMap[key].active
        }
      }
      return user
    })
