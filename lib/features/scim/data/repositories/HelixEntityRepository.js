...
class HelixEntityRepository extends EntityRepository {
  ...
  async addGroup(group, server, domain) {
    ...
    const [usernames, groupnames] = groomUsersAndGroups(model.members)
    ...
    const expectedData = model.members.length
  ...
  async getGroup(groupname, server, domain) {
    ...
    const group = new GroupModel(spec.Group, users.concat(groups))
