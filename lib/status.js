...
export async function validateCertAuth (caCertFile) {
  if (caCertFile) {
    ...
    return certCheckEnd(caCertFile, expiringInSeconds)
  }
...
export async function validatePfxFile (keyfile, passphrase) {
  ...
  return certCheckEndStdin(contents, expiringInSeconds)
}

function validatePfxMac (keyfile, passphrase) {
  return new Promise((resolve, reject) => {
    const passwd = passphrase ? `pass:${passphrase}` : 'pass:'
    const params = [
      'pkcs12', '-info', '-in', keyfile, '-passin', passwd, '-passout', passwd
    ]
    // eslint-disable-next-line no-unused-vars
    invokeOpenssl(params, (code, err, out) => {
      if (err.includes('MAC verified OK')) {
        resolve(out)
      } else {
        reject(new Error(err + out))
      }
