...
export function summarize (results) {
  return results.reduce((prev, curr) => {
    ...
    // check each property to see if it is 'ok'
    if (typeof curr === 'object') {
      ...
      // actually encountered properties and evaluated them, as some objects
      // (namely Error) does not have iterable properties
      if (hasProperties > 0) {
        return 'ok'
      }
    }
...
function certCheckEnd (x509file, seconds) {
  ...
  return new Promise((resolve, reject) => {
    invokeOpenssl(params, (code, err, out) => {
      if (code === 0) {
        // even the exit code of openssl is not a reliable API, making its use
        // extremely difficult
        if (out.includes('will expire')) {
          resolve('not ok')
        } else {
          resolve('ok')
        }
...
function invokeOpenssl (params, cb, input) {
  const stdout = []
  const stderr = []
  // When no input is piped in, give openssl an empty stdin (/dev/null) rather
  // than an inherited pipe. With OpenSSL 3.x, a pipe on stdin causes it to try
  // reading a passphrase from the console even when one was supplied via
  // -passin, failing with an "open_console: unknown ttyget" error.
  const stdio = input ? ['pipe', 'pipe', 'pipe'] : ['ignore', 'pipe', 'pipe']
  const proc = spawn('openssl', params, { stdio })
  if (input) {
