...
function readIdpMetadata (reader, samlOptions) {
  // Some services will advertise multiple certs but the last one is not
  // necessarily valid, so pass everything every time.
  const config = toPassportConfig(reader, { multipleCerts: true })
  // Merge IdP metadata with the configured and default settings. We must allow
  // the configured values to override anything the IdP provides in case
  // fine-tuning or work-arounds are needed. We could use Object.assign() accept
  // that it will copy over properties whose value is `undefined`, which is not
  // helpful for our purposes.
  for (const propName in samlOptions) {
    // copy the value if it is actually defined
