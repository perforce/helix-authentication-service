...
class DotenvSource {
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(this._filename, (err, data) => {
        if (err) {
          ...
          resolve(decodeValues(uppercaseKeys(dotenv.parse(data))))
        }
      })
    })
  }
  ...
  readSync() {
    try {
      ...
      return decodeValues(uppercaseKeys(dotenv.parse(data)))
    } catch (err) {
      if (err.code == 'ENOENT') {
...
// Setting names are conventionally UPPER_SNAKE_CASE, but TOML permits
// lowercase and camelCase keys and normalizes them to uppercase on read
// (see TomlSource.convertToEnvTop); do the same here so a .env file with
// lowercase setting names is not silently ignored (HAS-690).
function uppercaseKeys(parsed) {
